...
* Section : 001
* Assignment : 3
* Description :implementation file for the min heap   
*/
...
int MinHeap::getMin(){ // retrieves maximum element
	if (size != 0)
		return items[0];
	else
...
int MinHeap::removeMin(){ // retrieves and removes the maximum element
	if (size == 0)
		return 0;
	else {
		int min = items[0];
...
int MinHeap::getSize(){ // returns the number of elements in heap in O(1) time
	return size;
}
int MinHeap::getHeight(){ // returns the height of heap in O(1) time
	return log2(size - 1) +1;
}
int* MinHeap::getLessThan(int val){ // returns an array of integer elements
	int count = 0;
	...
	if (size == 0)
		...
		getLessThanHelper(greater, val, 0, count);		//call helper 
	return greater;
}

void MinHeap::getLessThanHelper(int*& greater, int val, int ind, int &count){
	greater [count++] = items  [ind];		//add elemnt

	if ( ind *2 +1 < size){			//has a left child 
		if (items [ind * 2 +1 ] < val) 		// left child less than the value entered
			getLessThanHelper (greater, val, ind*2 +1, count);	// 	call on the left child
 
	}
	if (ind *2 +2 < size){			//has a right child 
		if(items [ind * 2 +2 ] < val)		// right child less than the value entered
			getLessThanHelper (greater, val, ind*2 +2, count);		// 	call on the right child
	}
}
...
*/
void MinHeap::heapRebuild(){	// fix after the removal; this is similar to the rocess after the insertion
	int ind = 0;
	while (ind < size){
		if (items[ind *2 + 1] != 0 && items[ind * 2 + 2] != 0){		// has both right and left children
			if (items[ind *2 + 1] < items[ind] || items[ind * 2 + 2] < items[ind] ){	//one child is less than parent
				if 	(items[ind * 2 + 1] < items[ind * 2 + 2]){		// left is less
			...
			if (items[ind *2 + 1] < items[ind]){										//left child is less than parent
				swap(ind,ind * 2 + 1);		//swap elements
					ind = ind * 2 +1;		// update index for the next iteration
			}
...
void MinHeap::swap (int parent, int child){ // swap child with parent
